...
- **Offices annex / Assembly buildings:** 3D **isometric cutaway** sprites `public/HaulOfficesTop.png` and `public/HaulAssemblyTop.png` (roofs cut open to show interiors; doors/aprons toward the south for the dirt service track). SVG `<image>` uses `href` + `xlinkHref` and `preserveAspectRatio="xMidYMax meet"` so the 3D form is not squashed into flat blocks. Placement constants `OFFICES_BUILDING` / `ASSEMBLY_BUILDING` keep Offices left of Assembly and Assembly near the `HAUL_START` apron so road/safe-zone logic is unchanged. Labels + a small Orb-it badge on Assembly stay as SVG overlays.
