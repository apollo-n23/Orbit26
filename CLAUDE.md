...
| `auto-power` | Single master **ON** (not four cover→switch→charge buses); one click starts a fast parallel charge of all four buses |
...
- **Night sky (settled):** `.launch-prep-pad__sky` is a full-bleed plate (`public/LaunchPrepNightSky.jpg`, `object-fit: cover` / `object-position: center top`) plus a decorative `.launch-prep-pad__sky-fx` overlay — 8 CSS-only twinkle dots, 3 `LaunchPrepStarSparkle.png` pulses, and three drifting cirrus sprites (`LaunchPrepCloudA/B/C.png`, `launch-prep-sky-drift`). Sky has **no z-index** so tower (2), booster (3), and crane (4) stay in front. No React state, timers, or RAF. Detail: `docs/assets-launch-prep-overhaul.md`.
- **Pad deck (settled):** `.launch-prep-pad__ground` is a thin receding night apron (`public/LaunchPrepPadDeck.jpg`, height **22%** of the pad — not a steep ledge). The vehicle stands on a raised octagonal blast deck (`.lp-blast-pad` / `public/LaunchPrepBlastPad.png`) at the mated stack. Pad `min-height` is **32rem** (scene **40rem**) so the fairing fits above the booster.
- **Booster art (settled):** `Booster` with `showNose={false}` uses `AssemblyBooster.png` (same as manufacture/haul). Fairing/payload is a separate `.lp-payload` overlay after stack — not the CSS nosecone path. `.booster--launch-prep` is **2.8×10.25 rem** (up from 2.6×9.5) so the hull matches the 2.8rem fairing body; `.booster__sprite` uses `rotate(90deg)` so engines are down and the payload-mount face is up. Mated stack sits at `left: calc(42% + 3rem)` — to the right of the doubled mast, in the strongback clamps. Do not reintroduce CSS-shape nose/body/band rules for this class.
- **Launch tower art (settled):** Improved lattice mast (`LaunchPrepTowerMast.png?v=2`) at **3.51rem** (~30% over the previous 2.7rem). Slim strongback (`LaunchPrepStrongback.png?v=2`, 262×1454) at **2.3rem** with matching `aspect-ratio`. Base **8.3×2.6rem**. Strongback still mates via `--mate` rotate (`transform-origin: left center`, open **−24°**). A raised blast deck (`LaunchPrepBlastPad.png`) sits under the vehicle; the full-width ground is a thin receding apron, not a ledge. After the crane seats the fairing, `.lp-crane--clear` translates it off the right edge and fades it out (drone path unchanged). Mission-control tower in `LaunchSequenceScene` is **unchanged**. Detail: `docs/assets-launch-prep-tower.md`.
- **Crane / fairing / drone art (settled):** CSS-box crane, CSS-shape fairing, and CSS-rotor drone replaced by transparent PNG sprites inside the existing wrappers — `LaunchPrepCraneBase.png` / `Cab` / `Boom` / `Jib` / `Hook`, `LaunchPrepFairing.png`, `LaunchPrepDrone.png`. Phase classes `.lp-crane--phase-0` … `--phase-4` are unchanged. Boom transform is `translateX(-50%) rotate(...)` on every phase so the lattice stays centered on the old pivot; jib `object-position: left center` so a short phase-0 jib keeps its collar on the boom. Hook + hook-load stay at the **jib tip**, not the cable end. The cable is still a 2px CSS line. Seated fairing is **2.8rem** wide so the ogive body matches the **2.8rem** booster hull (`top: -6.9rem`, `z-index: 6` so it paints over the booster drop-shadow). To-be `payload-drone` is one click (`handleDroneDeploy`) + the drone sprite; rotors live in the PNG. Detail: `docs/assets-launch-prep-overhaul.md`.
...
- **Power-up terminal (settled):** baseline / Gemba / As-is is a vehicle-power-bus computer terminal (`.lp-power-term`, bezel `LaunchPrepPowerTerminal.png`). Four buses — Avionics, Flight computers, Telemetry, Range safety — each with a HUD icon, a hinged red safety cover (`LaunchPrepPowerCover.png`), a physical toggle (`LaunchPrepPowerToggleOff/On.png`), and a charge bar. Sequence is forced: lift the next cover → throw the switch → wait for that bus to charge to 100% before the next interlock releases. Hull lights (`.lp-power-lights`) arm one-by-one via `powerArmed.length` only when that bar hits 100%. To-be **auto-power** keeps the same terminal but replaces the covers with a single master **ON**; one click charges all four buses in parallel at a much faster rate, then completes. Fully powered / completed step keeps all four lights lit (and syncs `powerArmed` when `power-up` is already in `completedMachineIds`). Ambient `.lp-power-glow` still appears when the step is powered. HTML labels stay HTML — never baked into the sprites.
...
- Assets: **`public/`** for logos, photos, video, scene sprites, and static SVGs. Key recent sprites: `AccessCodeMonitor.png` (manufacture access-code TV), `AssemblyBooster.png` (bare booster on line + haul), `HaulOfficesTop.png` / `HaulAssemblyTop.png` (high-angle aerial buildings), haul field `HaulGrassField.jpg` / `HaulRunwayTile.jpg` / `HaulCoastStrip.jpg` / `HaulPadDeck.jpg` / `HaulTreeA-C.png` / `HaulErector*.png`, launch-prep tower/tank sprites `LaunchPrepTower*.png` / `LaunchPrepTankLox.png` / `LaunchPrepTankRp1.png`, launch-prep overhaul `LaunchPrepNightSky.jpg` / `LaunchPrepPadDeck.jpg` / `LaunchPrepBlastPad.png` / `LaunchPrepCloud*.png` / `LaunchPrepCrane*.png` / `LaunchPrepFairing.png` / `LaunchPrepDrone.png`, launch-prep power terminal `LaunchPrepPowerTerminal.png` / `LaunchPrepPowerCover.png` / `LaunchPrepPowerToggleOff.png` / `LaunchPrepPowerToggleOn.png` / `LaunchPrepPowerIcon*.png`. Also `OrbitLogo.png`, `AssemblyBG.jpg`, `Orbit26 Teaser.mp4`, Gemba step images `AssemblyStep.png` / `PadStep.png` / `PrepStep.png` / `MissContStep.png`, launch-prep `UpdateIcon*` / `UpgradeIconPump.jpg`. Full list in `docs/assets.md`.
